      ...
      <div className="flex border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        {tabs.map((tab) => (
          <button
            ...
            style={{
              color: active === tab.id ? "var(--on-surface)" : "var(--on-surface-dim)",
            }}
          >
            {tab.label}
            {active === tab.id && (
      ...
      {active === "abonnement" && (
        <div className="space-y-6 max-w-lg">
          ...
          <div className="card p-5 flex items-center gap-4">
            ...
            >
              <CreditCard size={20} style={{ color: "var(--on-surface-dim)" }} />
            </div>
                ...
                <tbody>
                  {mockPayments.map((p, i) => (
                    <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.03)" }}>
                      <td className="px-4 py-3" style={{ color: "var(--on-surface-dim)" }}>{p.date}</td>
                      <td className="px-4 py-3 font-mono-data font-semibold text-white">
                        {p.montant.toLocaleString("fr-FR")} FCFA
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className="font-bold px-1.5 py-0.5 rounded"
                          style={{ background: "rgba(0,255,136,0.1)", color: "#00CC6A" }}
                        >
                          {p.plan}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span style={{ color: p.statut === "Remboursé" ? "#F5A623" : "var(--on-surface-dim)" }}>
                          {p.statut}
                        </span>
                      </td>
                    </tr>
