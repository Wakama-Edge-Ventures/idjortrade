...
const pillActive: React.CSSProperties = { padding: "8px 14px", borderRadius: "999px", fontSize: "12px", fontWeight: 600, cursor: "pointer", background: "#00FF88", color: "#0A0E1A" };
  ...
  return (
    <div className="space-y-7">
      <div className="text-center">
        <h2 className="font-headline font-bold text-2xl text-white">⏰ Tes disponibilités</h2>
      </div>
      ...
      {/* Sessions */}
      <div>
        <p className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: "var(--on-surface-dim)" }}>
          Sessions préférées
        </p>
          ...
          {sessionsOptions.map((s) => {
            ...
            return (
              <button key={s.id} onClick={() => toggleSession(s.id)}
                ...
                <span className="text-xs font-mono-data" style={{ color: "var(--on-surface-dim)" }}>{s.hours}</span>
              </button>
            );
          })}
        </div>
      ...
      <div>
        <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: "var(--on-surface-dim)" }}>
          Objectif principal avec IdjorTrade
        </p>
          ...
          {objectifs.map((o) => {
            ...
            return (
              <button key={o.id} onClick={() => onChange({ objectifPrincipal: o.id })}
                ...
                <span className="text-sm font-semibold" style={{ color: active ? "#00FF88" : "var(--on-surface)" }}>{o.label}</span>
              </button>
            );
          })}
        </div>
