...
const cols = ["", "GRATUIT", "BASIC", "PRO", "TRADER"];
      ...
      <table className="w-full min-w-[600px] text-sm">
        <thead>
          <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            {cols.map((col, i) => (
              <th
                key={col}
                className="px-5 py-4 text-left text-xs font-bold tracking-widest"
                style={{
                  background: i === 3 ? "rgba(0,255,136,0.04)" : "var(--surface-mid)",
                  color: i === 3 ? "#00FF88" : "var(--on-surface-dim)",
        ...
        <tbody>
          {comparisonRows.map((row, ri) => (
            ...
            >
              <td className="px-5 py-3.5 text-xs font-medium" style={{ color: "var(--on-surface-dim)" }}>
                {row.feature}
              </td>
              {(["free", "basic", "pro", "trader"] as const).map((key, i) => (
                <td
                  key={key}
